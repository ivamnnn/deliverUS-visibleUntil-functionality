...
const availabiltyMakesSense = async (value, { req }) => {
  try {
    if(req.body.visibleUntil && value === false) {
      return Promise.reject(new Error('Cannot be unavailable and have visibility date.'))
    } else {
...
const create = [
  ...
  check('visibleUntil').optional({ checkNull: true, }).isDate().toDate(),
  check('availability').custom(availabiltyMakesSense),
